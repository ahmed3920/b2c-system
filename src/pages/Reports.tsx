...
const Reports = () => {
  ...
  const filteredTasks = tasks.filter(t => new Date(t.created_at) >= getDateFilter());
    ...
    <div className="min-h-screen bg-gradient-hero">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <motion.div
          ...
          className="bg-card rounded-xl p-6 shadow-lg mt-6"
        >
          <div className="flex items-center gap-2 mb-4">
          ...
          <div className="prose prose-sm max-w-none text-muted-foreground">
            <p>
              During the selected <strong>{dateRange}</strong> period, you have{" "}
              <strong>{metrics.total}</strong> tasks in total with a{" "}
              <strong>{metrics.completionRate}%</strong> completion rate.
            </p>
            {metrics.completed > 0 && (
              <p>
                You've completed <strong>{metrics.completed}</strong> tasks. 
                {typeData.length > 0 && (
                  <> Your most common task type is <strong>{typeData[0]?.name}</strong> with{" "}
                  <strong>{typeData[0]?.value}</strong> tasks.</>
                )}
              </p>
            )}
            {metrics.inProgress > 0 && (
              <p>
                You currently have <strong>{metrics.inProgress}</strong> tasks in progress 
                and <strong>{metrics.todo}</strong> tasks in your to-do list.
              </p>
            )}
          </div>
        </motion.div>
